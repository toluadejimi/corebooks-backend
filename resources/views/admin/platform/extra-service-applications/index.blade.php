...
@section('title', 'Service applications — '.config('app.name'))

@section('content')
<div class="adm-card" style="margin-bottom:1rem;">
    <h1 class="adm-page-title" style="font-size:1.35rem;">Service applications</h1>
    <p class="adm-page-desc">Requests submitted from the mobile app <strong>Services</strong> tab.</p>
</div>
            ...
            <tbody>
                @foreach ($applications as $a)
                    <tr style="border-bottom:1px solid var(--adm-border);">
                        ...
                        <td style="padding:0.65rem 0.5rem;">{{ $a->extraService?->title ?? '—' }}</td>
                        <td style="padding:0.65rem 0.5rem;"><span class="adm-role-pill">{{ $a->status }}</span></td>
                        <td style="padding:0.65rem 0.5rem;">{{ $a->created_at?->format('Y-m-d H:i') }}</td>
                        <td style="padding:0.65rem 0.5rem;">
                            <a href="{{ route('admin.platform.extra-service-applications.show', $a) }}" class="adm-btn adm-btn-ghost" style="height:34px;">Review</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
